class ServiceApplication < ActiveRecord::Base
	...
	belongs_to :application, class_name: "Spree::Application"
    ...
    validates :application_id, uniqueness: { scope: [:start_year, :end_year, :notes] }
    ...
    def update_name
      if self.name
        if application_name || !self.notes || self.notes != ""
